...
export interface DoctorSignUpExtra {
  name: string;
  center_name: string;
...
interface AuthContextType {
  ...
  loading: boolean;
  isRecovery: boolean;
  signUp: (email: string, password: string, extra: DoctorSignUpExtra) => Promise<{ error: string | null }>;
...
// Deep link back into the app after the OAuth browser flow (fluxiamedics://auth-callback)
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Doctor self-registration always sets is_doctor + center/specialty metadata,
  // consumed by the doctor_self_register RPC on first login (see
  // src/components/MedicsPanel.tsx's bootstrap flow, ported into AuthGateScreen).
  const signUp = async (email: string, password: string, extra: DoctorSignUpExtra) => {
    const { error } = await supabase.auth.signUp({
      email,
      password,
      options: {
        emailRedirectTo: 'https://fluxia-health.com/medics',
        data: {
          is_doctor: true,
          name: extra.name.trim(),
          center_name: extra.center_name.trim(),
          specialty: extra.specialty?.trim() || null,
        },
      },
    });
  ...
  const resetPassword = async (email: string) => {
    ...
    const { error } = await supabase.auth.resetPasswordForEmail(email, {
      redirectTo: 'https://fluxia-health.com/medics',
    });
    if (error) return { error: translateAuthError(error.message) };
    return { error: null };
  };
