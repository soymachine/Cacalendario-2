...
export default function PrivacyScreen({ onClose }: PrivacyScreenProps) {
  return (
    <div className="fixed inset-0 z-50 overflow-hidden" style={{ backgroundColor: D.bg }}>
      <div className="w-full max-w-md h-full mx-auto flex flex-col relative">
        <div className="flex-1 min-h-0 px-6 overflow-y-auto pb-10">
          ...
          <h1 className="text-xl font-black mb-4" style={{ color: D.text }}>
            POLÍTICA DE PRIVACIDAD
          </h1>
          <p className="text-xs mb-4" style={{ color: D.textMuted }}>
...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="mb-5">
      <h2 className="text-sm font-black mb-1" style={{ color: D.text }}>{title}</h2>
      <div className="text-sm leading-relaxed" style={{ color: D.textMuted }}>
        {children}
      </div>
    </div>
  );
