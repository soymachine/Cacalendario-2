// Native Deno/SubtleCrypto implementation of Web Push (RFC 8291 + RFC 8292).
// No npm:web-push dependency — works reliably in Supabase Edge Functions.
...
export async function buildVapidHeader(
  ...
  subject = 'mailto:noreply@fluxia-health.com',
): Promise<string> {
  ...
  const pubBytes = b64urlDecode(vapidPublicKey); // 65-byte uncompressed P-256
  ...
  const ecKey = await crypto.subtle.importKey(
    'jwk',
    {
      ...
      d: b64url(b64urlDecode(vapidPrivateKey)),
    },
...
async function encryptPayload(
  ...
  // PRK: HKDF-SHA-256(salt=auth_secret, IKM=shared_secret, info="WebPush: info\0"||ua_pub||as_pub)
  ...
  const prkBits = await crypto.subtle.deriveBits(
    {
      ...
      hash: 'SHA-256',
      salt: authSecret,
      info: new Uint8Array([...enc.encode('WebPush: info\x00'), ...uaPubBytes, ...asPubRaw]),
    },
    ikmKey,
    256,
  );
  ...
  // CEK (16 bytes) and Nonce (12 bytes) via HKDF with random salt
  ...
  // Padded plaintext: content || 0x02 (no extra padding)
